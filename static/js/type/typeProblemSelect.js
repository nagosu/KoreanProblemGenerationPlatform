...
let deleteProblems = []; // 선택된 문제들을 저장할 배열
...
function renderProblems(problems) {
  ...
  checkboxes.forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      ...
      updateDeleteProblems(); // 체크된 항목의 제목을 배열에 추가
    });
  });
...
function handlePagination(event) {
  ...
  fetchProblems(targetPage); // 페이지에 맞는 문제들 가져오기
  updatePaginationButtons(targetPage); // 페이지 버튼 상태 업데이트
}

// 페이지네이션 버튼 상태 업데이트 함수
function updatePaginationButtons(currentPage) {
  const totalPages = Math.ceil(dummyProblems.length / itemsPerPage); // 전체 페이지 수 계산
...
function moveToTypeProblemDetail(event) {
  const problem = event.currentTarget.innerText;
  ...
  resultPageUrl += `?typeProblem=${encodeURIComponent(
    problemType
...
function updateDeleteProblems() {
  const checkboxes = document.querySelectorAll(
    '.type-select-item input[type="checkbox"]'
  );

  deleteProblems.length = 0; // 배열 초기화
...
async function fetchProblems(page = 1) {
  const url = `/example?tab=${tab}&problemType=${problemType}&page=${page}&pageSize=10`; // 특정 문제 유형 조회 API 주소
  ...
  try {
    ...
    const data = {
      page: page,
      pageSize: 10,
      totalPages: Math.ceil(dummyProblems.length / 10),
      items: dummyProblems.slice(start, end),
    };
    console.log("특정 문제 유형 조회 성공", data);

    renderProblems(data.items); // 문제 렌더링
  } catch (e) {
    console.error(e);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetchProblems(1); // 페이지에 맞는 문제들 가져오기
