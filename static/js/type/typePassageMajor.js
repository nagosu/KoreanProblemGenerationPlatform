...
let tab = ""; // 탭 구분을 위한 변수
let previousPassageTypeMajor = ""; // 이전 지문 유형 (소)을 저장할 변수
let updatePassageTypeMajor = ""; // 수정할 지문 유형 (소)을 저장할 변수
let deletePassageTypeMajor = ""; // 삭제할 지문 유형 (소)을 저장할 변수
...
function openTypeEditModal(event) {
  const typePassageTitle = event.currentTarget
    .closest(".type-passage-major-item")
    .querySelector(".type-passage-major-title").textContent;
  previousPassageTypeMajor = typePassageTitle; // previousPassageTypeMajor에 저장
...
function moveToTypePassageMinor(event) {
  const passageTypeMajor = event.currentTarget
    .closest(".type-passage-major-item")
    .querySelector(".type-passage-major-title").textContent;
  ...
  if (currentUrl.includes("tab01")) {
    resultPageUrl = "../../../templates/tab01/type/tab01-typePassageMinor.html";
  } else if (currentUrl.includes("tab02")) {
    resultPageUrl = "../../../templates/tab02/type/tab02-typePassageMinor.html";
  }

  // 클릭한 지문 유형의 제목을 URL 파라미터로 추가
  resultPageUrl += `?passageTypeMajor=${encodeURIComponent(passageTypeMajor)}`;

  window.location.href = resultPageUrl;
}

// 지문 유형 (대) 페이지네이션 조회 API
async function fetchPassageTypeMajors(page = 1) {
  const url = `/example?tab=${tab}&page=${page}&pageSize=10`; // 지문 유형 (대) 페이지네이션 조회 API 주소
  ...
  try {
    ...
    const data = {
      page: page,
      pageSize: 10,
      totalPages: Math.ceil(dummyPassageMajor.length / 10),
      items: dummyPassageMajor.slice(start, end),
    };

    console.log("지문 유형 (대) 조회 성공", data);

    renderPassageTypeMajors(data.items); // 지문 유형 (대) 렌더링
  } catch (e) {
    console.error(e);
  }
}
...
// 지문 유형 (대) 렌더링
function renderPassageTypeMajors(passageTypeMajors) {
  const container = document.querySelector(".type-passage-major-container");
  container.innerHTML = ""; // 기존 지문 유형 (대) 초기화

  console.log(passageTypeMajors);

  passageTypeMajors.forEach((item) => {
    const passageTypeMajorItem = document.createElement("div");
    passageTypeMajorItem.classList.add("type-passage-major-item-container");
    passageTypeMajorItem.innerHTML = `
      <div class="type-passage-major-item">
        <div class="type-passage-major-title-container">
          <div class="dot"></div>
          <span
            class="type-passage-major-title"
            onclick="moveToTypePassageMinor(event)"
            >${item}</span>
        </div>
        <div class="type-passage-major-button-container">
          <div
    ...
    `;
    container.appendChild(passageTypeMajorItem);
  });
}

// 지문 유형 (대) 추가 API
async function addPassageTypeMajor() {
  // const url = "/"; // 지문 유형 (대) 추가 API 주소

  let createPassageTypeMajor = document.querySelector(
  ...
  try {
    ...
    //   body: JSON.stringify({
    //     tab: tab,
    //     createPassageTypeMajor: createPassageTypeMajor,
    //   }),
    // });
    // const data = await response.json();

    console.log("지문 유형 (대) 추가 성공", {
      tab: tab,
      createPassageTypeMajor: createPassageTypeMajor,
    });
...
// 지문 유형 (대) 수정 API
async function editPassageTypeMajor() {
  const url = "/"; // 지문 유형 (대) 수정 API 주소

  updatePassageTypeMajor = document.querySelector(
  ...
  try {
    ...
    //   body: JSON.stringify({
    //     tab: tab,
    //     previousPassageTypeMajor: previousPassageTypeMajor,
    //     updatePassageTypeMajor: updatePassageTypeMajor,
    //   }),
    // });
    // const data = await response.json();
...
// 지문 유형 (대) 삭제 API
async function removePassageTypeMajor() {
  const url = "/"; // 지문 유형 (대) 삭제 API 주소
  ...
  try {
    ...
    //   body: JSON.stringify({
    //     tab: tab,
    //     deletePassageTypeMajor: deletePassageTypeMajor,
    //   }),
    // });
    // const data = await response.json();

    console.log("지문 유형 (대) 삭제 성공", {
      tab: tab,
      deletePassageTypeMajor: deletePassageTypeMajor,
    });
...
function handlePagination(event) {
  ...
  fetchPassageTypeMajors(targetPage); // 해당 페이지의 문제 유형을 가져옴
  updatePaginationButtons(targetPage); // 페이지 버튼 상태 업데이트
}

// 페이지네이션 버튼 상태 업데이트 함수
function updatePaginationButtons(currentPage) {
  const totalPages = Math.ceil(dummyPassageMajor.length / 10); // 총 페이지 수 계산
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetchPassageTypeMajors(1); // 지문 유형 (대) 페이지네이션 조회 API 호출
});
