...
let newPassageFlow = ""; // 수정된 지문의 흐름 저장하는 변수
...
// 지문 수정 버튼 클릭 시
function editTypePassageDetail() {
...
function showToastSaveDone() {
  ...
  setTimeout(() => {
    ...
    if (currentUrl.includes("tab01")) {
      ...
      resultPageUrl =
        "../../../templates/tab02/type/tab02-typePassageMinor.html";
    }
...
function updatePassageInput() {
  ...
  newPassageFlow = spans[1] ? spans[1].innerHTML.trim() : "";
...
// 특정 지문 저장 API
async function savePassageDetail() {
  const url = "/example"; // 특정 지문 저장 API 주소
...
function displayPassageDetail(data) {
  newPassage = data.passage;
  newPassageFlow = data.passageFlow;
